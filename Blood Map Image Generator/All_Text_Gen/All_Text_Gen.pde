...
void draw() {
  ...
  if (drawTribeWords) {
    for (int i = 0; i < names.length; i++) {
      ...
      DrawWord("tribe/"+tSize+"/", names[i], names[i]);
    }
  }

  if (drawBloodWords) {
    for (int i =0; i < bloodnames.size(); i++) {
...
String[] GetBloodWordAndTribe(int i) {
  JSONObject b = bloodnames.getJSONObject(i);
